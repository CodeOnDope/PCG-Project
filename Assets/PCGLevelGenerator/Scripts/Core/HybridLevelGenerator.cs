...
using System.Linq;
using UnityEditor; // For Undo/EditorUtility
...
public class HybridLevelGenerator : MonoBehaviour
{
    ...
    // L-shape info class to store and manage L-shaped rooms
    [System.Serializable]
    private class LShapeInfo
    {
        public RectInt stemRect;
        public RectInt legRect;
        public RectInt overallBounds;

        // Define the LegPosition enum within the class
        ...
        // Add the legPosition property
        public LegPosition legPosition { get; private set; }

        // Internal enum used for connection logic
        ...
        // Constructor with position detection
        public LShapeInfo(RectInt stem, RectInt leg, RectInt overall)
        {
            stemRect = stem;
            legRect = leg;
            overallBounds = overall;

            // Determine the leg position relative to the stem
            DetermineLegPosition();
        }

        private void DetermineLegPosition()
        {
            // Default position
            legPosition = LegPosition.Undefined;

            // Check if leg is to the right of stem
            if (legRect.xMin >= stemRect.xMax - 1)
            {
                legPosition = LegPosition.Right;
            }
            // Check if leg is to the left of stem
            else if (legRect.xMax <= stemRect.xMin + 1)
            {
                legPosition = LegPosition.Left;
            }
            // Check if leg is above stem
            else if (legRect.yMin >= stemRect.yMax - 1)
            {
                legPosition = LegPosition.Above;
            }
            // Check if leg is below stem
            else if (legRect.yMax <= stemRect.yMin + 1)
            {
                legPosition = LegPosition.Below;
            }

            // If position still undefined, use center point comparison as fallback
            if (legPosition == LegPosition.Undefined)
            {
                Vector2 stemCenter = stemRect.center;
                Vector2 legCenter = legRect.center;

                float xDiff = legCenter.x - stemCenter.x;
                float yDiff = legCenter.y - stemCenter.y;

                if (Mathf.Abs(xDiff) > Mathf.Abs(yDiff))
                {
                    legPosition = xDiff > 0 ? LegPosition.Right : LegPosition.Left;
                }
                else
                {
                    legPosition = yDiff > 0 ? LegPosition.Above : LegPosition.Below;
                }
            }
        }

        // Helper to identify actual corner positions
        public List<Vector2Int> GetActualCorners()
        {
            List<Vector2Int> corners = new List<Vector2Int>();
            // Add all potential corners, then remove connection points
            // (detailed corner filtering logic based on connection type)
            return corners;
        }
    }
    ...
    private Dictionary<string, LShapeInfo> lShapeRooms = new Dictionary<string, LShapeInfo>();
    ...
    public void AddMissingCornerTiles()
    {
        ...
        // Special case for hybrid procedural mode - use our enhanced grid-based detection
        if (generationMode == GenerationMode.HybridProcedural)
        {
            ProcessHybridModeCorners(ref cornersPlaced);
        }
        else
        {
            // For other modes, use the original approach based on room bounds
            // First remove any existing outer corner tiles
            RemoveExistingOuterCornerTiles();

            // Process regular (non-L-shape) rooms
            foreach (var entry in placedRoomBounds)
            {
                string roomId = entry.Key;
                RectInt roomBounds = entry.Value;

                // Skip L-shaped rooms - we'll handle them separately
                if (lShapeRooms != null && lShapeRooms.ContainsKey(roomId))
                    continue;

                // Regular room corners
                PlaceRoomCorners(roomBounds, ref cornersPlaced);
            }

            // Process L-shaped rooms
            if (lShapeRooms != null)
            {
                foreach (var entry in lShapeRooms)
                {
                    string roomId = entry.Key;
                    LShapeInfo lShape = entry.Value;

                    // For L-shapes, we need to determine the actual corner positions
                    // based on the stem and leg rectangles
                    PlaceLShapeCorners(lShape, ref cornersPlaced);
                }
            }
        }
    ...
    // New method specifically for hybrid mode corner processing
    private void ProcessHybridModeCorners(ref int cornersPlaced)
    {
        Debug.Log("Using enhanced hybrid mode corner processing to fix L-shaped and template rooms");

        // First, clear any existing corner tiles
        RemoveExistingOuterCornerTiles();

        // STEP 1: Use a grid scanning approach to find all potential corner positions
        List<Vector3Int> potentialCornerPositions = new List<Vector3Int>();

        // Scan the entire grid to find wall tiles that might be corners
        for (int x = 1; x < levelWidth - 1; x++)
        {
            for (int y = 1; y < levelHeight - 1; y++)
            {
                ...
                // Check if this might be a corner position by examining its neighbors
                bool isCornerCandidate = IsOuterCornerCandidate(x, y);

                if (isCornerCandidate)
                {
                    potentialCornerPositions.Add(new Vector3Int(x, y, 0));
                }
            }
        }

        // STEP 2: For each potential corner position, determine the corner type and place the appropriate tile
        foreach (Vector3Int cornerPos in potentialCornerPositions)
        {
            WallType cornerType = DetermineCornerWallType(cornerPos.x, cornerPos.y);

            // Only process outer corner types
            if (cornerType == WallType.OuterTopLeft ||
                cornerType == WallType.OuterTopRight ||
                cornerType == WallType.OuterBottomLeft ||
                cornerType == WallType.OuterBottomRight)
            {
                // Place the corner tile
                PlaceCornerTileByType(cornerPos, cornerType, ref cornersPlaced);
            }
        }

        Debug.Log($"Enhanced hybrid corner detection found and processed {cornersPlaced} corner positions");
    }



    private void FixCornerTilesInHybridMode()
    {
        // This is a separate step to specifically address corner tiles in the hybrid mode
        // It runs after all other tiles have been placed but before corridor junctions

        // Only run for hybrid procedural mode
        if (generationMode != GenerationMode.HybridProcedural)
            return;

        int cornersFixed = 0;

        // Scan the grid to find all wall tiles that should be corners
        for (int x = 1; x < levelWidth - 1; x++)
        {
            for (int y = 1; y < levelHeight - 1; y++)
            {
                ...
                // Only look at wall tiles that have a specific pattern that indicates they're corners
                if (IsOuterCornerCandidate(x, y))
                {
                    WallType cornerType = DetermineCornerWallType(x, y);

                    // Only modify if it's one of the outer corner types
                    if (cornerType == WallType.OuterTopLeft ||
                        cornerType == WallType.OuterTopRight ||
                        cornerType == WallType.OuterBottomLeft ||
                        cornerType == WallType.OuterBottomRight)
                    {
                        Vector3Int pos = new Vector3Int(x, y, 0);
                        PlaceCornerTileByType(pos, cornerType, ref cornersFixed);
                    }
                }
            }
        }

        Debug.Log($"FixCornerTilesInHybridMode: Fixed {cornersFixed} additional corner tiles");
    }


    private WallType DetermineCornerWallType(int x, int y)
    {
        // Must be a wall tile
        if (!IsCoordInBounds(x, y) || grid[x, y] != TileType.Wall)
        ...
        bool hasWallAbove = IsCoordInBounds(x, y + 1) && grid[x, y + 1] == TileType.Wall;
        bool hasWallBelow = IsCoordInBounds(x, y - 1) && grid[x, y - 1] == TileType.Wall;
        ...
        bool hasFloorAbove = IsCoordInBounds(x, y + 1) && grid[x, y + 1] == TileType.Floor;
        bool hasFloorBelow = IsCoordInBounds(x, y - 1) && grid[x, y - 1] == TileType.Floor;
        ...
        // OUTER CORNER DETECTION - revised for better accuracy

        // TOP-LEFT OUTER CORNER: Wall to right and below, but not to left or above
        if ((!hasWallLeft && !hasWallAbove && hasWallRight && hasWallBelow) ||
            (hasFloorBottomRight && !hasFloorTopLeft && !hasFloorTopRight && !hasFloorBottomLeft))
            return WallType.OuterTopLeft;

        // TOP-RIGHT OUTER CORNER: Wall to left and below, but not to right or above
        if ((!hasWallRight && !hasWallAbove && hasWallLeft && hasWallBelow) ||
            (hasFloorBottomLeft && !hasFloorTopLeft && !hasFloorTopRight && !hasFloorBottomRight))
            return WallType.OuterTopRight;

        // BOTTOM-LEFT OUTER CORNER: Wall to right and above, but not to left or below
        if ((!hasWallLeft && !hasWallBelow && hasWallRight && hasWallAbove) ||
            (hasFloorTopRight && !hasFloorTopLeft && !hasFloorBottomRight && !hasFloorBottomLeft))
            return WallType.OuterBottomLeft;

        // BOTTOM-RIGHT OUTER CORNER: Wall to left and above, but not to right or below
        if ((!hasWallRight && !hasWallBelow && hasWallLeft && hasWallAbove) ||
            (hasFloorTopLeft && !hasFloorTopRight && !hasFloorBottomRight && !hasFloorBottomLeft))
            return WallType.OuterBottomRight;

        // Default to standard wall type
        return WallType.Default;
    }

    private bool IsOuterCornerCandidate(int x, int y)
    {
        // Must be a wall tile
        if (grid[x, y] != TileType.Wall)
            return false;

        // Check adjacent floor tiles (corners shouldn't have direct floor neighbors)
        bool hasFloorLeft = IsCoordInBounds(x - 1, y) && grid[x - 1, y] == TileType.Floor;
        bool hasFloorRight = IsCoordInBounds(x + 1, y) && grid[x + 1, y] == TileType.Floor;
        bool hasFloorAbove = IsCoordInBounds(x, y + 1) && grid[x, y + 1] == TileType.Floor;
        bool hasFloorBelow = IsCoordInBounds(x, y - 1) && grid[x, y - 1] == TileType.Floor;

        // Check diagonal floor tiles (outer corners often have diagonal floors)
        bool hasFloorTopLeft = IsCoordInBounds(x - 1, y + 1) && grid[x - 1, y + 1] == TileType.Floor;
        bool hasFloorTopRight = IsCoordInBounds(x + 1, y + 1) && grid[x + 1, y + 1] == TileType.Floor;
        bool hasFloorBottomLeft = IsCoordInBounds(x - 1, y - 1) && grid[x - 1, y - 1] == TileType.Floor;
        bool hasFloorBottomRight = IsCoordInBounds(x + 1, y - 1) && grid[x + 1, y - 1] == TileType.Floor;

        // Check wall tiles (corners should have specific wall configurations)
        bool hasWallLeft = IsCoordInBounds(x - 1, y) && grid[x - 1, y] == TileType.Wall;
        bool hasWallRight = IsCoordInBounds(x + 1, y) && grid[x + 1, y] == TileType.Wall;
        bool hasWallAbove = IsCoordInBounds(x, y + 1) && grid[x, y + 1] == TileType.Wall;
        bool hasWallBelow = IsCoordInBounds(x, y - 1) && grid[x, y - 1] == TileType.Wall;

        // An outer corner typically:
        // 1. Is a wall tile
        // 2. Has no adjacent floor tiles
        // 3. Has at least one adjacent wall tile
        // 4. Often has specific wall pattern (L-shape of walls)

        // No adjacent floor tiles is a key characteristic of outer corners
        if (hasFloorLeft || hasFloorRight || hasFloorAbove || hasFloorBelow)
            return false;

        // Basic pattern for different corner types:
        bool isTopLeftCorner = !hasWallLeft && !hasWallAbove && hasWallRight && hasWallBelow;
        bool isTopRightCorner = !hasWallRight && !hasWallAbove && hasWallLeft && hasWallBelow;
        bool isBottomLeftCorner = !hasWallLeft && !hasWallBelow && hasWallRight && hasWallAbove;
        bool isBottomRightCorner = !hasWallRight && !hasWallBelow && hasWallLeft && hasWallAbove;

        // Also consider locations where there are specific patterns of floor in diagonals
        bool hasTopLeftCornerPattern = hasFloorBottomRight && !hasFloorTopLeft && !hasFloorTopRight && !hasFloorBottomLeft;
        bool hasTopRightCornerPattern = hasFloorBottomLeft && !hasFloorTopLeft && !hasFloorTopRight && !hasFloorBottomRight;
        bool hasBottomLeftCornerPattern = hasFloorTopRight && !hasFloorTopLeft && !hasFloorBottomRight && !hasFloorBottomLeft;
        bool hasBottomRightCornerPattern = hasFloorTopLeft && !hasFloorTopRight && !hasFloorBottomRight && !hasFloorBottomLeft;

        // Combine wall patterns with diagonal floor patterns for more accurate detection
        return (isTopLeftCorner || isTopRightCorner || isBottomLeftCorner || isBottomRightCorner) ||
               (hasTopLeftCornerPattern || hasTopRightCornerPattern || hasBottomLeftCornerPattern || hasBottomRightCornerPattern);
    }

    // New utility method to place a corner tile based on its wall type
    private void PlaceCornerTileByType(Vector3Int pos, WallType cornerType, ref int cornersPlaced)
    {
        DirectionalTile tile = null;

        switch (cornerType)
        {
            case WallType.OuterTopLeft:
                tile = wallTileOuterTopLeft;
                break;
            case WallType.OuterTopRight:
                tile = wallTileOuterTopRight;
                break;
            case WallType.OuterBottomLeft:
                tile = wallTileOuterBottomLeft;
                break;
            case WallType.OuterBottomRight:
                tile = wallTileOuterBottomRight;
                break;
            default:
                return; // Not a corner type we're looking for
        }

        if (tile != null && tile.tile != null)
        {
            wallTilemap.SetTile(pos, tile.tile);
            if (tile.rotation != TileRotation.None)
                wallTilemap.SetTransformMatrix(pos, tile.GetRotationMatrix());
            cornersPlaced++;
        }
    }


    // Simplified corner placement for hybrid mode - places corners at exact positions
    private void PlaceExactRoomCorners(RectInt roomBounds, ref int cornersPlaced)
    {
        // Define exact corner positions (outside the room area)
        ...
        if (IsCoordInBounds(topLeft.x, topLeft.y) && grid[topLeft.x, topLeft.y] == TileType.Wall)
        {
        ...
        if (IsCoordInBounds(topRight.x, topRight.y) && grid[topRight.x, topRight.y] == TileType.Wall)
        {
        ...
        if (IsCoordInBounds(bottomLeft.x, bottomLeft.y) && grid[bottomLeft.x, bottomLeft.y] == TileType.Wall)
        {
        ...
        if (IsCoordInBounds(bottomRight.x, bottomRight.y) && grid[bottomRight.x, bottomRight.y] == TileType.Wall)
        {
    ...
    // Helper method to place corners for a rectangular room
    private void PlaceRoomCorners(RectInt roomBounds, ref int cornersPlaced)
    {
    ...
    // Helper method to place corners for an L-shaped room
    private void PlaceLShapeCorners(LShapeInfo lShape, ref int cornersPlaced)
    {
        RectInt stemRect = lShape.stemRect;
        RectInt legRect = lShape.legRect;

        switch (lShape.legPosition)
        {
            case LShapeInfo.LegPosition.Right:
                // L-shape with leg extending to the right of the stem
                // Stem corners (except top-right and/or bottom-right where it connects to leg)
                PlaceCornerTile(stemRect.xMin - 1, stemRect.yMax, wallTileOuterTopLeft, ref cornersPlaced);  // Top-left
                PlaceCornerTile(stemRect.xMin - 1, stemRect.yMin - 1, wallTileOuterBottomLeft, ref cornersPlaced);  // Bottom-left

                // Determine which stem corners are shared/connected to leg
                bool stemTopConnectedRight = (legRect.yMax >= stemRect.yMax && legRect.yMin <= stemRect.yMax);
                bool stemBottomConnectedRight = (legRect.yMax >= stemRect.yMin && legRect.yMin <= stemRect.yMin);

                if (!stemTopConnectedRight)
                    PlaceCornerTile(stemRect.xMax, stemRect.yMax, wallTileOuterTopRight, ref cornersPlaced);  // Top-right of stem

                if (!stemBottomConnectedRight)
                    PlaceCornerTile(stemRect.xMax, stemRect.yMin - 1, wallTileOuterBottomRight, ref cornersPlaced);  // Bottom-right of stem

                // Leg corners (always placed)
                PlaceCornerTile(legRect.xMax, legRect.yMax, wallTileOuterTopRight, ref cornersPlaced);  // Top-right
                PlaceCornerTile(legRect.xMax, legRect.yMin - 1, wallTileOuterBottomRight, ref cornersPlaced);  // Bottom-right
                break;

            case LShapeInfo.LegPosition.Left:
                // L-shape with leg extending to the left of the stem
                // Stem corners (except top-left and/or bottom-left where it connects to leg)
                PlaceCornerTile(stemRect.xMax, stemRect.yMax, wallTileOuterTopRight, ref cornersPlaced);  // Top-right
                PlaceCornerTile(stemRect.xMax, stemRect.yMin - 1, wallTileOuterBottomRight, ref cornersPlaced);  // Bottom-right

                // Determine which stem corners are shared/connected to leg
                bool stemTopConnectedLeft = (legRect.yMax >= stemRect.yMax && legRect.yMin <= stemRect.yMax);
                bool stemBottomConnectedLeft = (legRect.yMax >= stemRect.yMin && legRect.yMin <= stemRect.yMin);

                if (!stemTopConnectedLeft)
                    PlaceCornerTile(stemRect.xMin - 1, stemRect.yMax, wallTileOuterTopLeft, ref cornersPlaced);  // Top-left of stem

                if (!stemBottomConnectedLeft)
                    PlaceCornerTile(stemRect.xMin - 1, stemRect.yMin - 1, wallTileOuterBottomLeft, ref cornersPlaced);  // Bottom-left of stem

                // Leg corners (always placed)
                PlaceCornerTile(legRect.xMin - 1, legRect.yMax, wallTileOuterTopLeft, ref cornersPlaced);  // Top-left
                PlaceCornerTile(legRect.xMin - 1, legRect.yMin - 1, wallTileOuterBottomLeft, ref cornersPlaced);  // Bottom-left
                break;

            case LShapeInfo.LegPosition.Above:
                // L-shape with leg extending above the stem
                // Stem corners (except top-left and/or top-right where it connects to leg)
                PlaceCornerTile(stemRect.xMin - 1, stemRect.yMin - 1, wallTileOuterBottomLeft, ref cornersPlaced);  // Bottom-left
                PlaceCornerTile(stemRect.xMax, stemRect.yMin - 1, wallTileOuterBottomRight, ref cornersPlaced);  // Bottom-right

                // Determine which stem corners are shared/connected to leg
                bool stemLeftConnectedAbove = (legRect.xMax >= stemRect.xMin && legRect.xMin <= stemRect.xMin);
                bool stemRightConnectedAbove = (legRect.xMax >= stemRect.xMax && legRect.xMin <= stemRect.xMax);

                if (!stemLeftConnectedAbove)
                    PlaceCornerTile(stemRect.xMin - 1, stemRect.yMax, wallTileOuterTopLeft, ref cornersPlaced);  // Top-left of stem

                if (!stemRightConnectedAbove)
                    PlaceCornerTile(stemRect.xMax, stemRect.yMax, wallTileOuterTopRight, ref cornersPlaced);  // Top-right of stem
            ...
            case LShapeInfo.LegPosition.Below:
                // L-shape with leg extending below the stem
                // Stem corners (except bottom-left and/or bottom-right where it connects to leg)
                PlaceCornerTile(stemRect.xMin - 1, stemRect.yMax, wallTileOuterTopLeft, ref cornersPlaced);  // Top-left
                PlaceCornerTile(stemRect.xMax, stemRect.yMax, wallTileOuterTopRight, ref cornersPlaced);  // Top-right

                // Determine which stem corners are shared/connected to leg
                bool stemLeftConnectedBelow = (legRect.xMax >= stemRect.xMin && legRect.xMin <= stemRect.xMin);
                bool stemRightConnectedBelow = (legRect.xMax >= stemRect.xMax && legRect.xMin <= stemRect.xMax);

                if (!stemLeftConnectedBelow)
                    PlaceCornerTile(stemRect.xMin - 1, stemRect.yMin - 1, wallTileOuterBottomLeft, ref cornersPlaced);  // Bottom-left of stem

                if (!stemRightConnectedBelow)
                    PlaceCornerTile(stemRect.xMax, stemRect.yMin - 1, wallTileOuterBottomRight, ref cornersPlaced);  // Bottom-right of stem
            ...
            default: // Unknown or overlapping case
                // Fallback to treating it as a normal rectangle using its overall bounds
                Debug.LogWarning("L-shape with unknown configuration detected. Falling back to overall bounds.");
                PlaceRoomCorners(lShape.overallBounds, ref cornersPlaced);
                break;
        }
    }

    // Helper to place a single corner tile with rotation
    private void PlaceCornerTile(int x, int y, DirectionalTile cornerTile, ref int cornersPlaced)
    {
        if (!IsCoordInBounds(x, y))
            return;

        Vector3Int pos = new Vector3Int(x, y, 0);
        wallTilemap.SetTile(pos, cornerTile.tile);

        if (cornerTile.rotation != TileRotation.None)
            wallTilemap.SetTransformMatrix(pos, cornerTile.GetRotationMatrix());

        cornersPlaced++;
    }

    // Helper method to check if two points are close enough to consider connection
    private bool DoRectsOverlap(int x1, int y1, int x2, int y2, int tolerance = 2)
    {
        return Mathf.Abs(x1 - x2) <= tolerance && Mathf.Abs(y1 - y2) <= tolerance;
    }
    ...
    private void ClearGeneratedContentOnly()
    {
        ...
        if (lShapeRooms != null) lShapeRooms.Clear(); else lShapeRooms = new Dictionary<string, LShapeInfo>();
    }
    ...
    private void Initialize()
    {
        ...
        if (lShapeRooms == null) lShapeRooms = new Dictionary<string, LShapeInfo>(); else lShapeRooms.Clear();
    ...
    private bool GenerateFromSceneLayout(RoomNode[] roomNodes)
    {
        ...
        if (lShapeRooms == null) lShapeRooms = new Dictionary<string, LShapeInfo>();
        ...
        foreach (RoomNode node in roomNodes)
        {
            ...
            switch (node.roomType)
            {
                ...
                case NodeType.LShape:
                    ...
                    {
                        if (IsRectWithinBounds(stemRect) && IsRectWithinBounds(legRect))
                        {
                            CarveRectangle(stemRect, TileType.Floor);
                            CarveRectangle(legRect, TileType.Floor);

                            // Store L-shape specific information
                            lShapeRooms[node.roomId] = new LShapeInfo(stemRect, legRect, roomBounds);
    ...
    private void CreateRoomsInLeaves_Hybrid()
    {
        ...
        for (int i = 0; i < bspLeaves.Count; i++)
        {
            ...
            if (!roomCreated && pseudoRandom.NextDouble() < lShapeProbability)
            {
                if (TryCreateLShapeInLeaf(leaf, out RectInt stemRect, out RectInt legRect, out roomBounds))
                {
                    ...
                    // Ensure the overall bounds fully contain both the stem and leg
                    ...
                    // Use these explicit bounds to create a proper bounding rect
                    RectInt fullBounds = new RectInt(minX, minY, maxX - minX, maxY - minY);

                    // Store L-shape specific information with correct overall bounds
                    lShapeRooms[roomId] = new LShapeInfo(stemRect, legRect, fullBounds);
        ...
        Debug.Log($"Hybrid room creation complete. Rooms generated: {placedRoomBounds.Count}, L-Shapes: {lShapeRooms.Count}");
    }
    ...
    private bool TryCreateRectangleInLeaf(RectInt leaf, out RectInt roomRect)
    {
        ...
        int maxW = leaf.width - (2 * padding);
        int maxH = leaf.height - (2 * padding);

        if (maxW < minRoomSize || maxH < minRoomSize)
            return false;

        int w = pseudoRandom.Next(minRoomSize, maxW + 1);
        int h = pseudoRandom.Next(minRoomSize, maxH + 1);
        int x = leaf.x + padding + pseudoRandom.Next(0, maxW - w + 1);
        int y = leaf.y + padding + pseudoRandom.Next(0, maxH - h + 1);

        roomRect = new RectInt(x, y, w, h);
        return IsRectWithinBounds(roomRect);
    }
    ...
    private bool TryCreateLShapeInLeaf(RectInt leaf, out RectInt stemRect, out RectInt legRect, out RectInt overallBounds)
    {
        ...
        if (availW < minRoomSize || availH < minRoomSize)
            return false;

        int sW = pseudoRandom.Next(minRoomSize, availW + 1);
        int sH = pseudoRandom.Next(minRoomSize, availH + 1);
        int maxSX = availW - sW;
        int maxSY = availH - sH;
        ...
        int sRelX = pseudoRandom.Next(0, maxSX + 1);
        int sRelY = pseudoRandom.Next(0, maxSY + 1);
        stemRect = new RectInt(leaf.x + padding + sRelX, leaf.y + padding + sRelY, sW, sH);

        RectInt padLeaf = new RectInt(leaf.x + padding, leaf.y + padding, availW, availH);
        int spAb = padLeaf.yMax - stemRect.yMax;
        int spBe = stemRect.yMin - padLeaf.yMin;
        int spRi = padLeaf.xMax - stemRect.xMax;
        int spLe = stemRect.xMin - padLeaf.xMin;

        bool canAb = spAb >= minL;
        bool canBe = spBe >= minL;
        bool canRi = spRi >= minL;
        bool canLe = spLe >= minL;

        List<int> vAtt = new List<int>();
        if (canBe) vAtt.Add(0);
        if (canAb) vAtt.Add(1);

        List<int> hAtt = new List<int>();
        if (canLe) hAtt.Add(0);
        if (canRi) hAtt.Add(1);

        if (vAtt.Count == 0 && hAtt.Count == 0)
            return false;
        ...
        if (tryV && vAtt.Count == 0)
            tryV = false;
        if (!tryV && hAtt.Count == 0)
            tryV = true;

        int lW, lH, lX, lY;
        float ratio = Mathf.Clamp((float)pseudoRandom.Next((int)(minLLegRatio * 100), (int)(maxLLegRatio * 100) + 1) / 100f, minLLegRatio, maxLLegRatio);

        if (tryV)
        {
            bool attAb = vAtt[pseudoRandom.Next(vAtt.Count)] == 1;
            int availSp = attAb ? spAb : spBe;
            if (availSp < minL)
                return false;

            lH = pseudoRandom.Next(minL, availSp + 1);
            lW = (int)(sW * ratio);
            lW = Mathf.Clamp(lW, minL, sW);
            int maxLXOff = Mathf.Max(0, sW - lW);
            lX = stemRect.x + pseudoRandom.Next(0, maxLXOff + 1);
            lY = attAb ? stemRect.yMax : stemRect.yMin - lH;
        }
        else
        {
            bool attRi = hAtt[pseudoRandom.Next(hAtt.Count)] == 1;
            int availSp = attRi ? spRi : spLe;
            if (availSp < minL)
                return false;

            lW = pseudoRandom.Next(minL, availSp + 1);
            lH = (int)(sH * ratio);
            lH = Mathf.Clamp(lH, minL, sH);
            int maxLYOff = Mathf.Max(0, sH - lH);
            lY = stemRect.y + pseudoRandom.Next(0, maxLYOff + 1);
            lX = attRi ? stemRect.xMax : stemRect.xMin - lW;
        }

        if (lW < minL || lH < minL)
            return false;

        legRect = new RectInt(lX, lY, lW, lH);
    ...
    // Fixed TryPlaceRoomTemplate method with proper bounds recording
    private bool TryPlaceRoomTemplate(RectInt leaf, string roomId, out RectInt finalRoomBoundsForLeaf) // Renamed out param for clarity
    {
        finalRoomBoundsForLeaf = RectInt.zero; // This will be the tight floor bounds if successful

        if (roomTemplatePrefabs == null || roomTemplatePrefabs.Count == 0)
        {
            // Debug.LogWarning($"No room template prefabs assigned or prefab list is empty."); // Already logged by GetTemplateDimensions
            return false;
        }

        List<GameObject> validTemplates = roomTemplatePrefabs.Where(p => p != null).ToList();
        if (validTemplates.Count == 0)
        {
            // Debug.LogWarning($"All room template prefabs are null references."); // Already logged by GetTemplateDimensions
            return false;
        }
        ...
        {
            if (GetTemplateDimensions(prefab, out int tW, out int tH)) // GetTemplateDimensions gets overall cellBounds
            {
                templateSizes.Add((prefab, tW, tH, tW * tH));
            }
        }

        if (templateSizes.Count == 0)
        {
            Debug.LogWarning($"No valid room templates with dimensions found for room {roomId}.");
            return false;
        }

        templateSizes.Sort((a, b) => a.area.CompareTo(b.area)); // Smallest first

        int padding = Mathf.Max(0, (int)roomPadding);
        int availW = leaf.width - (2 * padding);
        int availH = leaf.height - (2 * padding);

        foreach (var templateData in templateSizes)
        {
            if (templateData.width > availW || templateData.height > availH)
                continue;

            int possibleX = availW - templateData.width + 1;
            int possibleY = availH - templateData.height + 1;
            ...
            if (possibleX > 0 && possibleY > 0)
            {
                ...
                // Attempt placement and get actualFloorBounds from the template
                if (PlaceSpecificRoomTemplate(templateData.prefab, new Vector2Int(startX, startY), out RectInt tightFloorBounds))
                {
                    // PlaceSpecificRoomTemplate returned true, meaning prefab was processed.
                    // Now check if it resulted in a valid floor area.
                    if (tightFloorBounds.width > 0 && tightFloorBounds.height > 0)
                    {
                        finalRoomBoundsForLeaf = tightFloorBounds; // This is the important bounds for the "room"
                        placedRoomBounds[roomId] = tightFloorBounds; // Store the TIGHT bounds for this room ID

                        Debug.Log($"Successfully placed template '{templateData.prefab.name}' in room {roomId}. Actual Floor Size: {tightFloorBounds.size}");
                        return true; // Successfully placed a template that defines a floor area
                    }
                    else
                    {
                        // Template was processed (e.g., walls carved) but it defined no floor area.
                        // Continue to try other templates if available.
                        Debug.LogWarning($"Template '{templateData.prefab.name}' processed for room {roomId}, but it defined no valid floor area. Trying next template.");
                    }
                }
                // If PlaceSpecificRoomTemplate returned false, it failed at a more basic level (e.g. bad prefab, out of level bounds).
            }
        }

        Debug.LogWarning($"Failed to place any template in room {roomId} that resulted in a valid floor area. Available leaf space: {availW}x{availH}");
        return false; // No suitable template with a floor area was placed
    }

    private bool PlaceSpecificRoomTemplate(GameObject templatePrefab, Vector2Int targetGridBottomLeft, out RectInt actualFloorBounds) // Renamed out param
    {
        actualFloorBounds = RectInt.zero; // Initialize
        RectInt templateCellBoundsRect;
        ...
        try
        {
            ...
            if (tm == null)
            {
                if (temp != null) { if (Application.isPlaying) Destroy(temp); else DestroyImmediate(temp); }
                return false;
            }
            tm.CompressBounds();
            ...
            templateCellBoundsRect = new RectInt(startX, startY, tW, tH);

            if (!IsRectWithinBounds(templateCellBoundsRect))
            {
                // If template's overall cell bounds are outside the level, it's a clear failure.
                if (temp != null) { if (Application.isPlaying) Destroy(temp); else DestroyImmediate(temp); }
                return false;
            }

            // For calculating tight floor bounds
            int minFloorX = int.MaxValue, minFloorY = int.MaxValue;
            int maxFloorX = int.MinValue, maxFloorY = int.MinValue;
            ...
            foreach (Vector3Int pos in bounds.allPositionsWithin)
            {
                ...
                if (tile != null)
                {
                    ...
                    if (IsCoordInBounds(gridX, gridY))
                    {
                        TileType type = TileType.Empty;
                        // Cache TileType lookups if not already done (optional performance improvement)
                        if (!tileTypeCache.TryGetValue(tile, out type))
                        {
                            if (tile == floorTile || floorTileVariants.Contains(tile)) type = TileType.Floor;
                            else if (tile == wallTile || wallTileVariants.Contains(tile)) type = TileType.Wall;
                            // Add more specific tile types from template if necessary
                            else
                            {
                                // If unknown, log once and treat as empty or a default
                                if (!reportedUnknownTiles.Contains(tile.name))
                                {
                                    Debug.LogWarning($"Unknown tile '{tile.name}' in template '{templatePrefab.name}'. Treating as Empty. Assign it as floor/wall or variant if it should be part of generation.");
                                    reportedUnknownTiles.Add(tile.name);
                                }
                                type = TileType.Empty; // Default for unknown
                            }
                            tileTypeCache[tile] = type;
                        }
                        ...
                        if (type != TileType.Empty)
                        {
                            ...
                            {
                                minFloorX = Mathf.Min(minFloorX, gridX);
                                minFloorY = Mathf.Min(minFloorY, gridY);
                                maxFloorX = Mathf.Max(maxFloorX, gridX);
                                maxFloorY = Mathf.Max(maxFloorY, gridY);
                                floorFound = true;
                            }
                        }
                    }
            ...
            if (floorFound)
            {
                actualFloorBounds = new RectInt(minFloorX, minFloorY, (maxFloorX - minFloorX) + 1, (maxFloorY - minFloorY) + 1);
            }
            // If !floorFound, actualFloorBounds remains RectInt.zero

            return true; // Template prefab was valid and processed (carved into grid).
                         // The presence of a usable floor is indicated by actualFloorBounds being non-zero.
        }
        catch (Exception ex)
        {
            Debug.LogError($"Template Placement Exception for '{templatePrefab?.name}': {ex.Message}\n{ex.StackTrace}");
            actualFloorBounds = RectInt.zero;
            return false;
        }
        finally
        {
    ...
    private bool GetTemplateDimensions(GameObject templatePrefab, out int width, out int height)
    {
        ...
        try
        {
            temp = Instantiate(templatePrefab);
            temp.SetActive(false);
            Tilemap tm = temp.GetComponentInChildren<Tilemap>(true);
    ...
    private bool TryCreateLShapeAt(Vector2Int centerPos, Vector2Int size, out RectInt stemRect, out RectInt legRect, out RectInt overallBounds)
    {
        ...
        if (size.x < minD * 2 || size.y < minD * 2)
            return false;

        int sW = size.x;
        int sH = Mathf.Max(minD, Mathf.RoundToInt(size.y * 0.7f));
        int lH = Mathf.Max(minD, size.y - sH);
        int lW = Mathf.Max(minD, Mathf.RoundToInt(size.x * 0.6f));

        int sX = centerPos.x - sW / 2;
        int sY = centerPos.y - size.y / 2;
        stemRect = new RectInt(sX, sY, sW, sH);

        int lX = stemRect.xMax - lW;
        int lY = stemRect.yMax;
        legRect = new RectInt(lX, lY, lW, lH);
    ...
    private void ConnectSceneNodes(RoomNode[] roomNodes)
    {
        ...
        foreach (RoomNode node in roomNodes)
        {
            if (node == null || string.IsNullOrEmpty(node.roomId) || !placedRoomBounds.ContainsKey(node.roomId))
                continue;
            ...
            if (node.connectedRooms != null)
            {
                foreach (RoomNode connectedNode in node.connectedRooms)
                {
                    ...
                    {
                        if (node.roomId == connectedNode.roomId)
    ...
    private void CreateCorridors_Procedural()
    {
        ...
        {
            RectInt nearUncon = default;
            RectInt srcCon = default;
            float minDistSq = float.MaxValue;

            foreach (RectInt conRoom in connected)
            {
                foreach (RectInt unconRoom in unconnected)
                {
                    float dSq = Vector2.SqrMagnitude(conRoom.center - unconRoom.center);
                    if (dSq < minDistSq)
                    {
                        minDistSq = dSq;
                        nearUncon = unconRoom;
            ...
            if (nearUncon != default && srcCon != default)
            {
                ConnectRects(srcCon, nearUncon);
                connected.Add(nearUncon);
                unconnected.Remove(nearUncon);
            }
            else
            {
                Debug.LogError("MST Corridor Error");
                break;
            }
        }
    ...
    private void CarveCorridorSegment(int startCoord, int endCoord, int fixedCoord, bool isHorizontal)
    {
        ...
        int halfW = corridorWidth <= 1 ? 0 : (corridorWidth - 1) / 2;
    ...
    private List<Vector2Int> GetFloorTilesInRect(RectInt rect)
    {
        ...
        int sX = Mathf.Max(rect.xMin, 0);
        int eX = Mathf.Min(rect.xMax, levelWidth);
        int sY = Mathf.Max(rect.yMin, 0);
        int eY = Mathf.Min(rect.yMax, levelHeight);
    ...
    private Vector3 GetWorldPosition(Vector2Int gridPos)
    {
        ...
        {
            return groundTilemap.CellToWorld((Vector3Int)gridPos) + groundTilemap.cellSize * 0.5f;
        }
        else
        {
            Debug.LogWarning("Ground Tilemap missing, using fallback world pos calc.");
    ...
    private void GetDirectionalTileWithTransform(WallType wallType, out TileBase tile, out Matrix4x4 transform)
    {
        ...
        // Print wall tile name for debugging (comment out for production)
        // if (tile != null)
        // {
    ...
    private void ApplyTilesToTilemap()
    {
        ...
        for (int x = 0; x < levelWidth; x++)
        {
            for (int y = 0; y < levelHeight; y++)
            {
                ...
                {
                    // In hybrid mode, we'll let our special corner handling take care of corners
                    bool isCorner = false;
                    if (useDirectionalWalls)
                    {
                        // For hybrid mode, just flag potential corners for later processing
                        if (generationMode == GenerationMode.HybridProcedural)
                        {
                            isCorner = IsOuterCornerCandidate(x, y);
                        }
                        else
                        {
                            // Original corner detection for other modes
                            foreach (var roomBounds in placedRoomBounds.Values)
                            {
        ...
        if (useDirectionalWalls)
        {
            ...
            // Additional step for hybrid mode to ensure corners are fixed
            if (generationMode == GenerationMode.HybridProcedural)
            {
                FixCornerTilesInHybridMode();
            }
    ...
    {
        if (prefab == null || availableSpots == null || parentHolder == null || availableSpots.Count == 0 || maxCount <= 0)
            return;

        int numSpawn = Mathf.Min(maxCount, availableSpots.Count);
        int spawned = 0;
    ...
    private void RunBSPSplit()
    {
        ...
        RectInt root = new RectInt(1, 1, levelWidth - 2, levelHeight - 2);

        if (root.width <= 0 || root.height <= 0)
        {
            Debug.LogError("Level Width/Height too small for BSP.");
            return;
        }

        var q = new Queue<KeyValuePair<RectInt, int>>();
        q.Enqueue(new KeyValuePair<RectInt, int>(root, 0));

        while (q.Count > 0)
        {
            var pair = q.Dequeue();
            RectInt node = pair.Key;
            int iter = pair.Value;

            if (iter >= maxIterations || ShouldStopSplitting(node))
            {
                bspLeaves.Add(node);
                continue;
            }

            if (TrySplitNode(node, out RectInt nA, out RectInt nB))
            {
                q.Enqueue(new KeyValuePair<RectInt, int>(nA, iter + 1));
                q.Enqueue(new KeyValuePair<RectInt, int>(nB, iter + 1));
            }
            else
            {
                bspLeaves.Add(node);
    ...
    {
        int minSplitSize = minRoomSize + Mathf.Max(1, (int)roomPadding) * 2;

        if (node.width < minSplitSize * 2 && node.height < minSplitSize * 2)
            return true;

        float aspect = node.width <= 0 || node.height <= 0 ? 1f :
                      (float)Mathf.Max(node.width, node.height) / Mathf.Max(1, Mathf.Min(node.width, node.height));
    ...
    private bool TrySplitNode(RectInt node, out RectInt nA, out RectInt nB)
    {
        ...
        bool canH = node.height >= (minRoomSize + (int)roomPadding) * 2 + 1;
        bool canV = node.width >= (minRoomSize + (int)roomPadding) * 2 + 1;
        ...
        {
            bool prefH = (node.height > node.width * 1.2f);
            bool prefV = (node.width > node.height * 1.2f);
            ...
            if (prefH)
                ...
                splitH = pseudoRandom.Next(0, 2) == 0;
        }

        int minSize = minRoomSize + (int)roomPadding;

        if (splitH)
        {
            int minY = node.y + minSize;
            int maxY = node.yMax - minSize;

            if (minY >= maxY)
                return false;

            int splitY = pseudoRandom.Next(minY, maxY);
            nA = new RectInt(node.x, node.y, node.width, splitY - node.y);
            nB = new RectInt(node.x, splitY, node.width, node.yMax - splitY);
        }
        else
        {
            int minX = node.x + minSize;
            int maxX = node.xMax - minSize;

            if (minX >= maxX)
                return false;

            int splitX = pseudoRandom.Next(minX, maxX);
            nA = new RectInt(node.x, node.y, splitX - node.x, node.height);
            nB = new RectInt(splitX, node.y, node.xMax - splitX, node.height);
        }
